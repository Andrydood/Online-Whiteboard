...
const chatServer = http.createServer(function(request, response) {
    console.log((new Date()) + ' Received request for ' + request.url);
});

chatServer.listen(8090, function() {
    console.log((new Date()) + 'Chat Server is listening on port 8090');
});
...
chatWSServer.on('request', function(request) {
    ...
    const connection = request.accept('whiteboard-chat', request.origin);
